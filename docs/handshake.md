...
The backend's library waits 3 s, resolves the name it was given, connects again and repeats the handshake. If the multiplexer is still down, or the name does not resolve, it tries again every 3 s. Once it is back, the backend is registered as if nothing had happened, under the same instance id.
