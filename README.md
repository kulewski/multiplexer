...
**A lightweight TCP message broker for service-to-service messaging.**
Not a socket multiplexer, not a WebSocket server: it is the broker your
services talk through, on a trusted network, for request/reply against a
pool of backends and publish/subscribe of events.

Peers connect over TCP, announce their peer type, and exchange typed
...
coordination between instances. It terminates no HTTP or WebSocket
...
An example: web servers on the left, two multiplexers in the middle, a
pool of C++ backends doing the expensive computation on the right. The web
servers are peers like any other, here the ones that ask; their own HTTP
or WebSocket traffic with browsers is theirs and never reaches the
multiplexer. Every peer is connected to both multiplexers; a request from
any web server reaches one backend, chosen round robin, and the answer
