...
Recorder::Recorder(const std::string &path, unsigned int payload_limit)
    : out_(path.c_str(), std::ios::out | std::ios::app | std::ios::binary), stream_(&out_, false),
      payload_limit_(payload_limit), failed_(!out_.good()) {
  if (failed_)
    MX_LOG(ERROR, LOWVERBOSITY, CTX("multiplexer.recorder") TEXT("cannot open " + path + "; not recording"));
}

void Recorder::header(boost::uint64_t multiplexer_id, const std::string &rules_sha1) {
  Record record;
  RecordingHeader *header = record.mutable_header();
  header->set_multiplexer_id(multiplexer_id);
  header->set_rules_sha1(rules_sha1);
  header->set_started_us(now_us());
  header->set_payload_limit(payload_limit_);
  _write(record);
}

void Recorder::peer(PeerEvent::Kind kind, boost::uint64_t peer_id, boost::uint32_t peer_type) {
  Record record;
...
void Recorder::routed(const MultiplexerMessage &msg, boost::uint32_t from_peer_type, boost::uint64_t recipient,
  ...
  if (payload_limit_ && msg.message().size() > payload_limit_) {
    routed->set_payload(msg.message().substr(0, payload_limit_));
...
void Recorder::_write(Record &record) {
  if (failed_)
    return;
  record.set_timestamp_us(now_us());
