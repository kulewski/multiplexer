...
void Task::_add_multiplexer_client_options(po::options_description &generic) {
  generic.add_options()("multiplexer,M", po::value(&multiplexers_)->composing(),
                        "multiplexer server address in a form [hostip]:port; "
                        "hostip defaults to 127.0.0.1\n"
                        "may be repeated");
}
...
void Task::parse_options(std::vector<std::string> &args) {
  ...
  cmdline_options.add(_options_description()).add(_hidden_options_description());
  po::parsed_options parsed =
      po::command_line_parser(args).options(cmdline_options).positional(_positional_options_description()).run();
  po::variables_map vm;
  po::store(parsed, vm);
  po::notify(vm);
}

void Task::print_help(std::ostream &out) { out << _options_description(); }
}; // namespace mxcontrol
