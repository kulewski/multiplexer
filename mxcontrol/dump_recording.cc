...
protected:
  virtual void _initialize_options_description(po::options_description &options) {
    options.add_options()("file", po::value(&file_), "the recording")(
...
int DumpRecording::run() {
  ...
  if (fd < 0) {
    std::cerr << "dump_recording: cannot open " << file_ << "\n";
    return 1;
  }
  mx::protobuf::FileMessageInputStream input(fd, true);
  multiplexer::Record record;
  while (input.read(record)) {
    if (type_ && !(record.has_routed() && record.routed().type() == type_))
      continue;
    if (peer_ && !involves(record, peer_))
      continue;
