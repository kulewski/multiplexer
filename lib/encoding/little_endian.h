...
template <> struct LittleEndian<std::uint32_t> {
  template <typename Encoder> static void encode(Encoder &enc, std::uint32_t n) {
  ...
  template <typename Decoder> static void decode(Decoder &dec, std::uint32_t &n) {
