...
#define Assert(w)                                                                                  \
  do {                                                                                             \
    if (!(w)) ::mx::_AssertionFailed(__FILE__, __LINE__, __PRETTY_FUNCTION__, MX_PP_STRINGIZE(w)); \
  } while (0)

#define AssertMsg(w, args...)                                                                            \
  do {                                                                                                   \
    if (!(w)) ::mx::_AssertionFailed(__FILE__, __LINE__, __PRETTY_FUNCTION__, MX_PP_STRINGIZE(w), args); \
  } while (0)
